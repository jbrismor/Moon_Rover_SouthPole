...
#     # Initialize plotting with 2 subplots
#     plt.ion()
...
#             ax2.clear()
#             ax2.plot(train_lengths, label="Train Length", color="orange")
#             if eval_iterations:
#                 ax2.plot(eval_iterations, eval_lengths, label="Eval Length", color="red")
...
# Import your environment code
from Moon_Rover import LunarRover3DEnv  # or use NormalizeObservation if needed
...
# beta_anneal_callback.py should be in your PYTHONPATH
from beta_anneal_callback import BetaAnnealCallback

# Custom metrics function to collect 'is_success'
from ray.rllib.evaluation.metrics import collect_episodes, summarize_episodes
def custom_metrics_fn(episodes):
    return {
        "is_success": [
...
#############################################
# 2) TRAINING MANAGER CLASS WITH PLOTTING   #
#############################################
class TrainingManager:
    def __init__(self, stop_iters=1300, checkpoint_dir="./checkpoints", plot_dir="./training_plots"):
        self.stop_iters = stop_iters
        self.checkpoint_dir = checkpoint_dir
        self.plot_dir = plot_dir
        os.makedirs(self.checkpoint_dir, exist_ok=True)
    ...
    def _update_plots(self, ax1, ax2, live_plot_path):
        """Helper function to clear and update the live plots."""
        # Update reward plot
        ax1.clear()
        ax1.plot(self.train_rewards, label="Train Reward", color="lightblue")
        if self.eval_rewards:
            ax1.plot(self.eval_iterations, self.eval_rewards, label="Eval Reward", linestyle="--", color="orange")
        ax1.set_title("Reward Progress")
        ax1.set_xlabel("Iteration")
        ax1.set_ylabel("Reward")
        ax1.legend()

        # Update episode lengths plot
        ax2.clear()
        ax2.plot(self.train_lengths, label="Train Episode Length", color="lightblue")
        if self.eval_lengths:
            ax2.plot(self.eval_iterations, self.eval_lengths, label="Eval Episode Length", color="orange", linestyle="--")
        ax2.set_title("Episode Lengths")
    ...
    def train(self):
        ...
        # Set up live plotting with 2 subplots (side by side)
        plt.ion()
        ...
        for i in range(self.stop_iters):
            ...
            # Update plots every 10 iterations
            if (i + 1) % 10 == 0:
                self._update_plots(ax1, ax2, live_plot_path)
            ...
            if (i + 1) % 50 == 0:
                eval_msg = f"{self.eval_rewards[-1]:.1f}" if self.eval_rewards else "N/A"
                print(f"Iter={i+1}, Train Reward={self.train_rewards[-1]:.1f}, Eval Reward={eval_msg}")
